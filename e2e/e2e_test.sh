#!/usr/bin/env bash
# e2e_test.sh — End-to-end tests for specai installer
#
...
test_version_command() {
    ...
    if echo "$output" | grep -q "specai"; then
...
test_dry_run_preset_full() {
    log_test "Dry-run with --preset full-specai"

    output=$($BINARY install --preset full-specai --dry-run 2>&1) || true

    assert_output_contains "$output" "Preset: full-specai" "Shows full-specai preset"
}
...
test_preset_minimal_components() {
    log_test "Preset minimal with persona=custom produces only sdd-memory component"
    ...
    # The component list should contain sdd-memory
    assert_output_contains "$output" "sdd-memory" "Minimal preset includes sdd-memory"
...
test_preset_minimal_with_default_persona_includes_persona() {
    log_test "Preset minimal with default persona (specai) includes persona"

    # Persona is now decoupled from preset — default SpecAI persona is
    ...
    assert_output_contains "$components_line" "sdd-memory" "Minimal includes sdd-memory"
    assert_output_contains "$components_line" "persona" "Minimal + default SpecAI persona includes persona"
}
...
test_preset_ecosystem_components() {
    ...
    # ecosystem-only (without persona) = sdd-memory, sdd, skills, context7
    local components_line
    components_line=$(echo "$output" | grep "Components order:")

    assert_output_contains "$components_line" "sdd-memory" "Ecosystem includes sdd-memory"
...
test_preset_full_with_custom_persona_excludes_persona() {
    log_test "Preset full-specai with persona=custom excludes persona"

    # Persona is decoupled — picking persona=custom skips persona install
    # even when the preset is full-specai.
    output=$($BINARY install --preset full-specai --persona custom --agent claude-code --dry-run 2>&1) || true

    local components_line
    components_line=$(echo "$output" | grep "Components order:")

    assert_output_contains "$components_line" "sdd-memory" "Full + persona=custom keeps sdd-memory"
...
test_preset_full_components() {
    log_test "Preset full-specai includes core and optional SpecAI components"

    output=$($BINARY install --preset full-specai --agent claude-code --dry-run 2>&1) || true

    local components_line
    components_line=$(echo "$output" | grep "Components order:")

    assert_output_contains "$components_line" "sdd-memory" "Full includes sdd-memory"
    ...
    assert_output_contains "$components_line" "claude-theme" "Full includes Claude SpecAI theme"
    assert_output_contains "$components_line" "opencode-specai-logo" "Full includes OpenCode SpecAI logo"
}

test_dry_run_full_preset_persona_before_sdd() {
    log_test "Dry-run: persona appears before sdd-memory and sdd in component order"

    output=$($BINARY install --preset full-specai --agent opencode --dry-run 2>&1) || true
    ...
    assert_output_contains "$components_line" "sdd-memory" "Full preset has sdd-memory"
    assert_output_contains "$components_line" "sdd" "Full preset has sdd"

    # Verify ordering: persona before sdd-memory, persona before sdd
    # Extract the order string and check persona comes first
    local order_str
    order_str=$(echo "$components_line" | sed 's/.*Components order: *//')

    local persona_idx sddMemory_idx sdd_idx
    persona_idx=$(echo "$order_str" | tr ',' '\n' | grep -n '^persona$' | cut -d: -f1)
    sddMemory_idx=$(echo "$order_str" | tr ',' '\n' | grep -n '^sdd-memory$' | cut -d: -f1)
    sdd_idx=$(echo "$order_str" | tr ',' '\n' | grep -n '^sdd$' | cut -d: -f1)

    if [ -n "$persona_idx" ] && [ -n "$sddMemory_idx" ] && [ "$persona_idx" -lt "$sddMemory_idx" ]; then
        log_pass "Persona ($persona_idx) before sdd-memory ($sddMemory_idx)"
    else
        log_fail "Persona must appear before sdd-memory in component order: $order_str"
    fi
...
test_preset_no_legacy_theme_in_any_preset() {
    ...
    for preset in minimal ecosystem-only full-specai; do
...
test_preset_custom_no_components() {
    ...
    local components_line
    components_line=$(echo "$output" | grep "Components order:")
    assert_output_not_contains "$components_line" "sdd-memory" "Custom preset without components excludes sdd-memory"
...
test_preset_custom_explicit_components() {
    ...
    output=$($BINARY install --preset custom --agent claude-code --component sdd-memory --component sdd --component skills --dry-run 2>&1) || true

    local components_line
    components_line=$(echo "$output" | grep "Components order:")
    assert_output_contains "$components_line" "sdd-memory" "Custom + explicit components includes sdd-memory"
...
test_dry_run_component_sddMemory() {
    log_test "Dry-run with --component sdd-memory"
    output=$($BINARY install --agent claude-code --component sdd-memory --dry-run 2>&1) || true
    assert_output_contains "$output" "sdd-memory" "Shows sdd-memory component"
}
...
test_cc_sddMemory_injection() {
    log_test "Claude Code: sdd-memory injection (MCP + CLAUDE.md)"
    cleanup_test_env

    if $BINARY install --agent claude-code --component sdd-memory --persona neutral 2>&1; then
        # MCP config
        assert_file_exists "$HOME/.claude/mcp/sdd-memory.json" "sdd-memory.json MCP config"
        assert_file_contains "$HOME/.claude/mcp/sdd-memory.json" '"command"' "sdd-memory.json has 'command' key"
        assert_file_contains "$HOME/.claude/mcp/sdd-memory.json" 'sdd-memory' "sdd-memory.json command points to sdd-memory binary (absolute or relative)"
        assert_valid_json "$HOME/.claude/mcp/sdd-memory.json" "sdd-memory.json is valid JSON"

        # CLAUDE.md section
        assert_file_exists "$HOME/.claude/CLAUDE.md" "CLAUDE.md exists"
        assert_file_contains "$HOME/.claude/CLAUDE.md" "specai:sdd-memory-protocol" "CLAUDE.md has sdd-memory-protocol section marker"
        assert_file_contains "$HOME/.claude/CLAUDE.md" "mem_save" "CLAUDE.md has real SddMemory content (mem_save)"
        assert_file_size_min "$HOME/.claude/CLAUDE.md" 500 "CLAUDE.md has substantial content"
    else
        log_fail "sdd-memory install command failed"
    fi
}
...
test_cc_sdd_injection() {
    ...
    if $BINARY install --agent claude-code --component sdd --persona neutral 2>&1; then
        assert_file_exists "$HOME/.claude/CLAUDE.md" "CLAUDE.md exists"
        assert_file_contains "$HOME/.claude/CLAUDE.md" "specai:sdd-orchestrator" "CLAUDE.md has SDD section marker"
        ...
        assert_file_contains "$HOME/.claude/agents/sdd-explore.md" "mcp__plugin_sddMemory_sddMemory__mem_save" "Claude explore sub-agent includes SddMemory save"
        ...
        assert_file_contains "$HOME/.claude/agents/sdd-apply.md" "mcp__plugin_sddMemory_sddMemory__mem_search" "Claude apply sub-agent includes SddMemory search"
        assert_file_contains "$HOME/.claude/agents/sdd-apply.md" "mcp__plugin_sddMemory_sddMemory__mem_get_observation" "Claude apply sub-agent includes SddMemory read"
        assert_file_contains "$HOME/.claude/agents/sdd-apply.md" "mcp__plugin_sddMemory_sddMemory__mem_save" "Claude apply sub-agent includes SddMemory save"
        assert_file_contains "$HOME/.claude/agents/sdd-apply.md" "mcp__plugin_sddMemory_sddMemory__mem_update" "Claude apply sub-agent includes SddMemory update"
        ...
        assert_file_contains "$HOME/.claude/agents/sdd-verify.md" "mcp__plugin_sddMemory_sddMemory__mem_search" "Claude verify sub-agent includes SddMemory search"
        assert_file_contains "$HOME/.claude/agents/sdd-verify.md" "mcp__plugin_sddMemory_sddMemory__mem_get_observation" "Claude verify sub-agent includes SddMemory read"
        assert_file_contains "$HOME/.claude/agents/sdd-verify.md" "mcp__plugin_sddMemory_sddMemory__mem_save" "Claude verify sub-agent includes SddMemory save"
    else
        log_fail "SDD install command failed"
    fi
}

test_cc_persona_specai() {
    log_test "Claude Code: persona injection (specai)"
    cleanup_test_env

    if $BINARY install --agent claude-code --component persona --persona specai 2>&1; then
        assert_file_exists "$HOME/.claude/CLAUDE.md" "CLAUDE.md exists"
        assert_file_contains "$HOME/.claude/CLAUDE.md" "specai:persona" "CLAUDE.md has persona section marker"
        assert_file_contains "$HOME/.claude/CLAUDE.md" "Senior Architect" "SpecAI persona has 'Senior Architect'"
        assert_file_size_min "$HOME/.claude/CLAUDE.md" 200 "Persona section is substantial"
        # Output-style file
        assert_file_exists "$HOME/.claude/output-styles/specai.md" "Output-style file exists"
        assert_file_contains "$HOME/.claude/output-styles/specai.md" "name: SpecAI" "Output-style has YAML frontmatter"
        assert_file_contains "$HOME/.claude/output-styles/specai.md" "keep-coding-instructions: true" "Output-style keeps coding instructions"
        ...
        assert_file_contains "$HOME/.claude/settings.json" "SpecAI" "settings.json outputStyle is SpecAI"
    else
        log_fail "persona (specai) install command failed"
    fi
}
...
test_cc_persona_custom_does_nothing() {
    ...
    if $BINARY install --agent claude-code --component persona --persona custom 2>&1; then
        ...
        assert_file_not_exists "$HOME/.claude/output-styles/specai.md" "No output-style for custom"
    else
...
test_cc_skills_full() {
    log_test "Claude Code: skills injection (full-specai = 10 foundation skills)"
    cleanup_test_env

    if $BINARY install --agent claude-code --component skills --preset full-specai --persona neutral 2>&1; then
        local skills_dir="$HOME/.claude/skills"
        assert_dir_exists "$skills_dir" "Claude skills directory"
...
test_cc_custom_no_skills_flag_installs_nothing() {
    ...
    if $BINARY install --agent claude-code --preset custom --component skills --persona neutral 2>&1; then
        local skills_dir="$HOME/.claude/skills"
        # --component skills auto-resolves sdd as a hard dependency (graph: skills → sdd → sdd-memory).
...
test_cc_custom_sdd_plus_skills() {
    ...
    if $BINARY install --agent claude-code --preset custom --component sdd-memory --component sdd --component skills --skills go-testing,branch-pr --persona neutral 2>&1; then
        local skills_dir="$HOME/.claude/skills"
        assert_dir_exists "$skills_dir" "Claude skills directory"
...
test_cc_theme_injection() {
    ...
    if $BINARY install --agent claude-code --component theme --persona neutral 2>&1; then
        ...
        assert_file_contains "$settings" 'specai-kanagawa' "Has specai-kanagawa theme"
        assert_valid_json "$settings" "settings.json is valid JSON"
    else
...
test_oc_sddMemory_injection() {
    log_test "OpenCode: sdd-memory injection (opencode.json)"
    cleanup_test_env

    if $BINARY install --agent opencode --component sdd-memory --persona neutral 2>&1; then
        ...
        assert_file_contains "$settings" '"sdd-memory"' "Has sdd-memory MCP entry"
        assert_file_contains "$settings" '"command"' "Has command key"
        assert_file_contains "$settings" '"type": "local"' "SddMemory uses local MCP type"
        assert_valid_json "$settings" "opencode.json is valid JSON"

        # Fallback safety: AGENTS.md must include sdd-memory protocol section.
        assert_file_exists "$agents_md" "OpenCode AGENTS.md"
        assert_file_contains "$agents_md" 'specai:sdd-memory-protocol' "AGENTS.md has sdd-memory-protocol section"
        assert_file_contains "$agents_md" 'mem_save' "AGENTS.md has memory protocol content"
    else
        log_fail "OpenCode sdd-memory install command failed"
    fi
}
...
test_oc_persona_specai() {
    log_test "OpenCode: persona injection (specai)"
    cleanup_test_env

    if $BINARY install --agent opencode --component persona --persona specai 2>&1; then
        local agents_md="$HOME/.config/opencode/AGENTS.md"
        assert_file_exists "$agents_md" "AGENTS.md exists"
        assert_file_contains "$agents_md" "Senior Architect" "SpecAI persona has 'Senior Architect'"
        assert_file_size_min "$agents_md" 200 "AGENTS.md has substantial content"
    else
        log_fail "OpenCode persona (specai) install command failed"
    fi
}
...
test_oc_skills_full() {
    log_test "OpenCode: skills injection (full-specai = 10 foundation skills)"
    cleanup_test_env

    if $BINARY install --agent opencode --component skills --preset full-specai --persona neutral 2>&1; then
        local skill_dir="$HOME/.config/opencode/skills"
        assert_dir_exists "$skill_dir" "OpenCode skill directory"
...
test_qwen_sddMemory_injection() {
    log_test "Qwen: sdd-memory injection (settings.json)"
    cleanup_test_env

    if $BINARY install --agent qwen-code --component sdd-memory --persona neutral 2>&1; then
        ...
        log_fail "Qwen sdd-memory install command failed"
    fi
}

test_qwen_sddMemory_idempotency() {
    log_test "Qwen: sdd-memory injection is idempotent"
    cleanup_test_env
    ...
    $BINARY install --agent qwen-code --component sdd-memory --persona neutral > /dev/null 2>&1 || true
    if [ ! -f "$settings" ]; then
...
test_full_preset_claude_code() {
    log_test "Full-specai preset: Claude Code (all components coexist)"
    cleanup_test_env

    # full-specai has: sdd-memory, sdd, skills, context7, persona, permissions
    # SddMemory needs binary install (go install) — may fail but injection components
    ...
    if $BINARY install --agent claude-code --component sdd --component persona --component skills --component context7 --component permissions --component theme --preset full-specai --persona specai 2>&1; then
        local claude_md="$HOME/.claude/CLAUDE.md"
        local settings="$HOME/.claude/settings.json"

        # CLAUDE.md should have all 3 sections coexisting
        assert_file_exists "$claude_md" "CLAUDE.md exists"
        assert_file_contains "$claude_md" "specai:sdd-orchestrator" "Has SDD section"
        assert_file_contains "$claude_md" "specai:persona" "Has persona section"
...
test_full_preset_opencode() {
    log_test "Full-specai preset: OpenCode (all components coexist)"
    cleanup_test_env

    if $BINARY install --agent opencode --component sdd-memory --component sdd --component persona --component skills --component context7 --component permissions --component theme --preset full-specai --persona specai 2>&1; then
        local settings="$HOME/.config/opencode/opencode.json"
        local agents_md="$HOME/.config/opencode/AGENTS.md"
        ...
        # AGENTS.md for persona + sdd-memory (SDD orchestrator is in opencode.json for OpenCode, NOT AGENTS.md)
        assert_file_exists "$agents_md" "AGENTS.md exists"
        assert_file_contains "$agents_md" "Senior Architect" "SpecAI persona"
        assert_file_contains "$agents_md" "specai:sdd-memory-protocol" "AGENTS.md has sdd-memory protocol"
        assert_no_duplicate_section "$agents_md" "sdd-memory-protocol" "No duplicate sdd-memory section in AGENTS.md"
        # SDD orchestrator for OpenCode lives in opencode.json as an agent definition (not AGENTS.md)
        assert_file_contains "$settings" '"specai-orchestrator"' "opencode.json has specai-orchestrator agent"
        ...
        assert_file_not_contains "$agents_md" "<!-- specai:sdd-orchestrator -->" "AGENTS.md has no SDD section marker (opencode uses json agent)"
...
test_minimal_preset_opencode_only_sddMemory_no_persona() {
    log_test "Minimal preset: OpenCode (sdd-memory only, no persona side effect)"
    cleanup_test_env
    ...
    if $BINARY install --agent opencode --preset minimal --persona custom 2>&1; then
        ...
        assert_file_contains "$settings" '"sdd-memory"' "OpenCode has sdd-memory MCP"

        # Minimal preset should NOT silently install persona.
        if [ -f "$agents_md" ]; then
            assert_file_not_contains "$agents_md" "specai:persona" "No persona marker in minimal preset"
...
test_minimal_preset_claude_only_sddMemory() {
    log_test "Minimal preset: Claude Code (only sdd-memory, nothing else)"
    cleanup_test_env

    if $BINARY install --agent claude-code --preset minimal --persona custom 2>&1; then
        # SddMemory should be installed (MCP + CLAUDE.md)
        assert_file_exists "$HOME/.claude/CLAUDE.md" "CLAUDE.md exists"
        assert_file_contains "$HOME/.claude/CLAUDE.md" "specai:sdd-memory-protocol" "SddMemory protocol section"

        # SDD should NOT be in CLAUDE.md
        assert_file_not_contains "$HOME/.claude/CLAUDE.md" "specai:sdd-orchestrator" "No SDD in minimal"
        # Persona should NOT be in CLAUDE.md
        assert_file_not_contains "$HOME/.claude/CLAUDE.md" "specai:persona" "No persona in minimal"
...
test_ecosystem_both_agents() {
    ...
    if $BINARY install --agent claude-code --agent opencode --component sdd --component skills --component context7 --preset ecosystem-only --persona neutral 2>&1; then
        ...
        assert_file_contains "$HOME/.claude/CLAUDE.md" "specai:sdd-orchestrator" "Claude has SDD"
...
test_content_claude_md_sections_substantial() {
    ...
    # Install SDD + persona + sdd-memory (all inject into CLAUDE.md)
    $BINARY install --agent claude-code --component sdd --component persona --persona specai 2>&1 || true
    $BINARY install --agent claude-code --component sdd-memory --persona specai 2>&1 || true
...
test_content_skills_are_real() {
    ...
    $BINARY install --agent claude-code --component skills --preset full-specai --persona neutral 2>&1 || true
...
test_content_mcp_json_valid() {
    ...
    $BINARY install --agent claude-code --component sdd-memory --persona neutral 2>&1 || true
...
test_idempotent_persona_claude() {
    ...
    $BINARY install --agent claude-code --component persona --persona specai 2>&1 || true
    $BINARY install --agent claude-code --component persona --persona specai 2>&1 || true
...
test_idempotent_sddMemory_claude() {
    log_test "Idempotency: sdd-memory on Claude Code (no duplicate sections)"
    cleanup_test_env
    ...
    local claude_md="$HOME/.claude/CLAUDE.md"
    if [ -f "$claude_md" ]; then
        assert_no_duplicate_section "$claude_md" "sdd-memory-protocol" "No duplicate sdd-memory section after 2 runs"

        # Also check MCP JSON is identical
        local mcp_file="$HOME/.claude/mcp/sdd-memory.json"
        if [ -f "$mcp_file" ]; then
            assert_valid_json "$mcp_file" "sdd-memory.json still valid after 2 runs"
        fi
    else
        log_fail "CLAUDE.md not found"
    fi
...
test_gemini_sddMemory_tools_flag() {
    log_test "Gemini: sdd-memory injection uses --tools=agent"
    cleanup_test_env

    if $BINARY install --agent gemini-cli --component sdd-memory --persona neutral 2>&1; then
        ...
        assert_file_contains "$settings" '"sdd-memory"' "Has sdd-memory entry"
        assert_file_contains "$settings" '"--tools=agent"' "SddMemory args include --tools=agent"
        assert_valid_json "$settings" "settings.json is valid JSON"
    else
        log_fail "Gemini sdd-memory install command failed"
    fi
}

# ─── Codex parity tests ───────────────────────────────────────────────────────

test_codex_sddMemory_injection() {
    log_test "Codex: sdd-memory injection writes config.toml + instruction files"
    cleanup_test_env

    if $BINARY install --agent codex --component sdd-memory --persona neutral 2>&1; then
        local config_toml="$HOME/.codex/config.toml"
        local instructions="$HOME/.codex/sdd-memory-instructions.md"
        local compact="$HOME/.codex/sdd-memory-compact-prompt.md"

        assert_file_exists "$config_toml" "Codex config.toml"
        assert_file_contains "$config_toml" '[mcp_servers.sdd-memory]' "config.toml has [mcp_servers.sdd-memory]"
        assert_file_contains "$config_toml" 'command = ".*sdd-memory"' "config.toml has correct command"
        ...
        assert_file_exists "$instructions" "sdd-memory-instructions.md"
        assert_file_contains "$instructions" 'mem_save' "Instructions have memory protocol content"

        assert_file_exists "$compact" "sdd-memory-compact-prompt.md"
        assert_file_contains "$compact" 'FIRST ACTION REQUIRED' "Compact prompt has required sentinel"
    else
        log_fail "Codex sdd-memory install command failed"
    fi
}

test_codex_sddMemory_idempotent() {
    log_test "Codex: sdd-memory injection is idempotent (no duplicate blocks)"
    cleanup_test_env

    $BINARY install --agent codex --component sdd-memory --persona neutral 2>&1 || true
    $BINARY install --agent codex --component sdd-memory --persona neutral 2>&1 || true

    local config_toml="$HOME/.codex/config.toml"
    if [ -f "$config_toml" ]; then
        local count
        count=$(grep -c '\[mcp_servers\.sdd-memory\]' "$config_toml" || true)
        if [ "$count" -ne 1 ]; then
            log_fail "config.toml has $count [mcp_servers.sdd-memory] blocks after 2 runs (want exactly 1)"
        else
            log_pass "config.toml has exactly 1 [mcp_servers.sdd-memory] block after 2 runs"
        fi
    else
...
test_idempotent_full_claude() {
    ...
    $BINARY install --agent claude-code --component sdd --component persona --component context7 --component permissions --component theme --preset full-specai --persona specai 2>&1 || true
    local first_md_hash
    first_md_hash=$(md5sum "$HOME/.claude/CLAUDE.md" 2>/dev/null | cut -d' ' -f1)
    # Snapshot settings.json for semantic comparison (sdd-memory setup may reorder
    # top-level keys on re-run — see sdd-memory binary's non-deterministic map
...
test_edge_persona_switch() {
    log_test "Edge case: switching persona from specai to neutral"
    cleanup_test_env

    # First install with specai (Rioplatense language present)
    $BINARY install --agent claude-code --component persona --persona specai 2>&1 || true
    assert_file_contains "$HOME/.claude/CLAUDE.md" "Senior Architect" "First install: specai persona"
...
test_edge_persona_switch_preserves_sections_opencode() {
    ...
    # Step 1: Install full stack with specai
    $BINARY install --agent opencode --component persona --component sdd-memory --component sdd --persona specai 2>&1 || true

    local agents_md="$HOME/.config/opencode/AGENTS.md"
    assert_file_exists "$agents_md" "AGENTS.md after full install"
    assert_file_contains "$agents_md" "specai:sdd-memory-protocol" "SddMemory section present before switch"
    ...
    assert_file_contains "$agents_md" "specai:sdd-memory-protocol" "SddMemory section survived persona switch"
    assert_no_duplicate_section "$agents_md" "sdd-memory-protocol" "No duplicate sdd-memory after switch"
}
...
test_windsurf_persona_and_sdd_content() {
    ...
    if $BINARY install --agent windsurf --component persona --component sdd --persona specai 2>&1; then
        ...
        assert_file_contains "$rules" "specai:sdd-orchestrator" "SDD orchestrator marker present"
        assert_file_contains "$rules" "skill_resolution" "SDD has skill_resolution field"
        assert_file_contains "$rules" "SddMemory Topic Key" "SDD has SddMemory Topic Key section"
        assert_file_contains "$rules" "specai:sdd-memory-protocol" "SddMemory protocol marker present"
...
test_codex_context7_not_in_toml() {
    ...
    # Install sdd-memory first (creates config.toml) then try context7
    $BINARY install --agent codex --component sdd-memory --persona neutral 2>&1 || true

    local config_toml="$HOME/.codex/config.toml"
    if [ -f "$config_toml" ]; then
        # SddMemory created config.toml — now try context7 on top
        ...
        # config.toml wasn't created by sdd-memory, so no context7 either
        $BINARY install --agent codex --component context7 --persona neutral 2>&1 || true
...
test_integrity_sdd_orchestrator_in_opencode_json() {
    log_test "Integrity: opencode.json contains specai-orchestrator agent after SDD install"
    cleanup_test_env

    if $BINARY install --agent opencode --component sdd --persona neutral 2>&1; then
        local settings="$HOME/.config/opencode/opencode.json"
        assert_file_exists "$settings" "opencode.json exists"
        assert_file_contains "$settings" '"specai-orchestrator"' "Has specai-orchestrator agent"
...
test_integrity_full_preset_all_skills_nonempty() {
    ...
    if $BINARY install --agent opencode --component sdd --component skills --preset full-specai --persona specai 2>&1; then
        local skill_dir="$HOME/.config/opencode/skills"
        local all_ok=true
        local empty_count=0
...
test_integrity_sdd_orchestrator_agent_structure() {
    log_test "Integrity: specai-orchestrator agent has required fields in opencode.json"
    cleanup_test_env

    if $BINARY install --agent opencode --component sdd --persona specai 2>&1; then
        local settings="$HOME/.config/opencode/opencode.json"
        assert_file_contains "$settings" '"specai-orchestrator"' "Has specai-orchestrator"
...
test_integrity_skills_plus_sdd_coexist() {
    ...
    if $BINARY install --agent opencode --component sdd --component skills --preset full-specai --persona neutral 2>&1; then
        local skill_dir="$HOME/.config/opencode/skills"
        ...
        # opencode.json should have specai-orchestrator as the base coordinator
        assert_file_contains "$HOME/.config/opencode/opencode.json" '"specai-orchestrator"' "specai-orchestrator present"
...
test_backup_created_on_install() {
    ...
    if $BINARY install --agent opencode --component permissions --persona neutral 2>&1; then
        local backup_count
        backup_count=$(find "$HOME/.specai/backups" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | wc -l | tr -d ' ')
...
test_backup_contains_original_files() {
    ...
    if $BINARY install --agent opencode --component permissions --persona neutral 2>&1; then
        local latest_backup
        latest_backup=$(find "$HOME/.specai/backups" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | sort | tail -1)
...
if [ "${RUN_FULL_E2E:-0}" = "1" ]; then
    ...
    # Category 6b: Gemini/Codex sdd-memory parity
    test_gemini_sddMemory_tools_flag
    test_codex_sddMemory_injection
    test_codex_sddMemory_idempotent
