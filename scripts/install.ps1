...
<#
.SYNOPSIS
    SpecAI — Install Script for Windows
    Ecosystem, Frameworks, Workflows for AI coding agents.

.DESCRIPTION
    Downloads and installs the specai binary for Windows.
    Supports installation via Go or pre-built binary from GitHub Releases.

.EXAMPLE
    # Run directly:
    irm https://raw.githubusercontent.com/KevG1t/specai/main/scripts/install.ps1 | iex

    # Or download and run:
    Invoke-WebRequest -Uri https://raw.githubusercontent.com/KevG1t/specai/main/scripts/install.ps1 -OutFile install.ps1
    .\install.ps1
...
function Show-Banner {
    ...
    Write-Host ""
    Write-Host "  SpecAI — Ecosystem, Frameworks, Workflows" -ForegroundColor DarkGray
    Write-Host ""
}
...
function Get-LatestVersion {
    ...
    try {
        $response = Invoke-RestMethod -Uri $url -Headers @{ "User-Agent" = "specai-installer" }
    } catch {
...
function Install-ViaBinary {
    ...
    $tmpDir = Join-Path $env:TEMP "specai-install-$(Get-Random)"
    ...
    try {
        ...
        if (-not $installDir) {
            $installDir = Join-Path $env:LOCALAPPDATA "specai\bin"
        }
...
function Test-Installation {
    ...
    $locations = @(
        (Join-Path $env:LOCALAPPDATA "specai\bin\$BINARY_NAME.exe")
    )
    if ($gopath) {
    ...
    foreach ($loc in $locations) {
        ...
        # self-update explicitly via SPECAI_NO_SELF_UPDATE so the check is a
        # pure version read even if the binary is older.
        $env:SPECAI_NO_SELF_UPDATE = "1"
        $versionOutput = & $loc --version 2>&1
        Remove-Item Env:SPECAI_NO_SELF_UPDATE -ErrorAction SilentlyContinue
