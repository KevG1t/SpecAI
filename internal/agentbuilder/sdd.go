...
// Example: <!-- specai:custom-agent:my-skill -->
const markerFormat = "<!-- specai:custom-agent:%s -->"
...
func buildSDDBlock(agent *GeneratedAgent, marker string) string {
	...
	endMarker := fmt.Sprintf("<!-- /specai:custom-agent:%s -->", agent.Name)
	return marker + "\n" + body + endMarker
}
...
func replaceBlock(content, marker, newBlock string) string {
	endMarker := fmt.Sprintf("<!-- /specai:custom-agent:%s -->", extractName(marker))
...
// Example: "<!-- specai:custom-agent:my-skill -->" → "my-skill"
func extractName(marker string) string {
	prefix := "<!-- specai:custom-agent:"
	suffix := " -->"
