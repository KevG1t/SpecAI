...
func TestExecute_DevBuildIsSkipped(t *testing.T) {
	...
	// specai (DevBuild) MUST appear as UpgradeSkipped with a ManualHint.
	var devResult *ToolUpgradeResult
	for i := range report.Results {
		if report.Results[i].ToolName == "specai" {
			r := report.Results[i]
			devResult = &r
		}
	}
	if devResult == nil {
		t.Fatalf("specai (DevBuild) must appear in Results — was not found")
	}
	if devResult.Status != UpgradeSkipped {
		t.Errorf("specai DevBuild Status = %q, want UpgradeSkipped", devResult.Status)
	}
...
// A DevBuild tool (e.g. specai with version="dev") MUST appear in UpgradeReport.Results
...
func TestExecute_DevBuildSurfacedAsSkipped(t *testing.T) {
	...
	// specai (DevBuild) MUST appear in results as UpgradeSkipped.
	var devResult *ToolUpgradeResult
	for i := range report.Results {
		if report.Results[i].ToolName == "specai" {
			r := report.Results[i]
	...
	if devResult == nil {
		t.Fatalf("specai DevBuild must appear in Results as UpgradeSkipped, but was not found")
	}
