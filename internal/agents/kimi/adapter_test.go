...
func TestAdapter_PostInstallMessage(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name:     "Unix paths",
			os:       "linux",
			expected: "/.kimi/agents/modism.yaml",
		},
		{
			name:     "Windows paths",
			os:       "windows",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			specaiYaml := filepath.Join(homeDir, ".kimi", "agents", "modism.yaml")
			...
			if (runtime.GOOS == "windows" && tt.os == "windows") || (runtime.GOOS != "windows" && tt.os == "linux") {
				...
				// Verify path is quoted (specifically the modism.yaml path)
				quotedExpected := `"` + specaiYaml + `"`
