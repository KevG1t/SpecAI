...
func TestStripLegacyPersonaBlock_LegacyBlockBeforeMarkersStripped(t *testing.T) {
	...
	if !strings.Contains(result, "<!-- specai:persona -->") {
		t.Fatal("stripped result missing specai marker section")
	}
}
...
func TestStripLegacyPersonaBlock_MixedZone_OnlyOneFingerprint_PreMarker(t *testing.T) {
	...
	// exist only inside a specai marker block.
	//
	...
	markerWithOtherFingerprints := "<!-- specai:persona -->\n## Personality\n\nSenior Architect, 15+ years experience.\n<!-- /specai:persona -->\n"
...
func TestStripLegacyPersonaBlock_UserContentBeforeAndAfterMarkersPreserved(t *testing.T) {
	...
	if !strings.Contains(result, "# Custom section") {
		t.Fatal("content after specai markers must be preserved")
	}
}
...
func TestStripLegacyATLBlock_ATLBlockThenMarkers_StripsATLKeepsMarkers(t *testing.T) {
	...
	if !strings.Contains(result, "<!-- specai:sdd-orchestrator -->") {
...
// --- MigrateMarkers tests ---

func TestMigrateMarkers_NoLegacyMarkers_ReturnsUnchanged(t *testing.T) {
	input := "# My Config\n\nSome content.\n<!-- specai:persona -->\nNew content.\n<!-- /specai:persona -->\n"
	result := MigrateMarkers(input)
	if result != input {
		t.Fatalf("no legacy markers: expected unchanged result:\ngot:  %q\nwant: %q", result, input)
	}
}
...
func TestMigrateMarkers_LegacyOpenMarker_Migrated(t *testing.T) {
	...
	result := MigrateMarkers(input)
	if result != want {
		t.Fatalf("legacy markers: expected migration:\ngot:  %q\nwant: %q", result, want)
	}
}
...
func TestMigrateMarkers_MixedMarkersAllMigrated(t *testing.T) {
	...
	result := MigrateMarkers(input)
	if result != want {
		t.Fatalf("mixed markers: expected all migrated:\ngot:  %q\nwant: %q", result, want)
	}
}
...
func TestMigrateMarkers_Idempotent(t *testing.T) {
	...
	if once != twice {
		t.Fatalf("migrate idempotent: second call changed result:\nfirst:  %q\nsecond: %q", once, twice)
	}
}
