...
func MigrateMarkers(content string) string {
	content = strings.ReplaceAll(content, legacyMarkerPrefix, markerPrefix)
...
// <!-- specai: --> marker). A fingerprint that exists only inside a marker
...
// written by the standalone ATL installer before specai superseded it.
...
// <!-- specai:sdd-orchestrator --> section, so keeping both wastes ~150
