...
//   - GET /releases/download/…   → a real .tar.gz containing "sdd-memory" binary
...
func makeServerWithFakeTarGz(t *testing.T, version string) *httptest.Server {
	t.Helper()
	tarContent := buildFakeTarGz(t, "sdd-memory")
	...
	for _, goos := range []string{"linux", "darwin"} {
		for _, goarch := range []string{"amd64", "arm64"} {
			name := sddMemoryArchiveName(version, goos, goarch)
...
func buildFakeZip(t *testing.T, binaryName string) []byte {
	...
	content := []byte("fake sdd-memory.exe binary")
...
// --- TestSDDMemoryAssetURL ---

func TestSDDMemoryAssetURL(t *testing.T) {
	tests := []struct {
		name       string
		version    string
		goos       string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			url := sddMemoryAssetURL("https://github.com", tt.version, tt.goos, tt.goarch)
			if !strings.Contains(url, tt.wantSubstr) {
				t.Errorf("sddMemoryAssetURL(%s, %s) = %q, want it to contain %q", tt.goos, tt.goarch, url, tt.wantSubstr)
			}
			if !strings.HasSuffix(url, tt.wantExt) {
				t.Errorf("sddMemoryAssetURL(%s) = %q, want suffix %q", tt.goos, url, tt.wantExt)
			}
		})
	}
}

// --- TestSDDMemoryInstallDir ---

func TestSDDMemoryInstallDir(t *testing.T) {
	tests := []struct {
		name       string
		goos       string
		wantSubstr string
		...
		{
			name:       "windows returns LOCALAPPDATA sdd-memory bin",
			goos:       "windows",
			wantSubstr: "sdd-memory",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			dir := sddMemoryInstallDir(tt.goos)
			if !strings.Contains(dir, tt.wantSubstr) {
				t.Errorf("sddMemoryInstallDir(%s) = %q, want it to contain %q", tt.goos, dir, tt.wantSubstr)
			}
		})
	}
}
...
func TestDownloadLatestBinaryLinux(t *testing.T) {
	...
	origClient := sddMemoryHTTPClient
	origBaseURL := sddMemoryGitHubBaseURL
	sddMemoryHTTPClient = server.Client()
	sddMemoryGitHubBaseURL = server.URL
	t.Cleanup(func() {
		sddMemoryHTTPClient = origClient
		sddMemoryGitHubBaseURL = origBaseURL
	})

	// Override install dir to a temp directory (avoids needing root).
	tmpDir := t.TempDir()
	origInstallDirFn := sddMemoryInstallDirFn
	sddMemoryInstallDirFn = func(goos string) string { return tmpDir }
	t.Cleanup(func() { sddMemoryInstallDirFn = origInstallDirFn })
...
func TestDownloadLatestBinarySkipsLatestReleaseWithoutBinaryAssets(t *testing.T) {
	...
	checksums := ""
	for _, goarch := range []string{"amd64", "arm64"} {
		checksums += makeChecksumsTxt(sddMemoryArchiveName(binaryVersion, "linux", goarch), tarContent)
	}
			...
			json.NewEncoder(w).Encode([]map[string]any{
				...
				},
				{
					...
					"assets": []map[string]string{
						{"name": "checksums.txt"},
						{"name": "sdd-memory_" + binaryVersion + "_linux_amd64.tar.gz"},
					},
				},
			})
		case strings.HasSuffix(r.URL.Path, "checksums.txt"):
			w.Header().Set("Content-Type", "text/plain")
			fmt.Fprint(w, checksums)
		case strings.Contains(r.URL.Path, "/releases/download/v"+binaryVersion+"/sdd-memory_"+binaryVersion+"_linux_"):
			w.Header().Set("Content-Type", "application/octet-stream")
			w.WriteHeader(http.StatusOK)
			w.Write(tarContent)
		default:
...
func TestDownloadLatestBinaryFallsBackToAnonymousWhenTokenGets403(t *testing.T) {
	...
	for _, goos := range []string{"linux", "darwin"} {
		for _, goarch := range []string{"amd64", "arm64"} {
			checksums += makeChecksumsTxt(sddMemoryArchiveName(version, goos, goarch), tarContent)
		}
	}
...
// --- TestSDDMemoryChecksumVerification ---

func TestSDDMemoryChecksumVerification(t *testing.T) {
	const version = "1.0.0"

	tarContent := buildFakeTarGz(t, "sdd-memory")
	correctDigest := sha256Hex(tarContent)
	archiveName := sddMemoryArchiveName(version, "linux", normalizeArch(runtime.GOARCH))
...
// --- TestSDDMemoryExpectedChecksumFor ---

func TestSDDMemoryExpectedChecksumFor(t *testing.T) {
	tests := []struct {
		name          string
		content       string
		filename      string
	...
	}{
		{
			...
			content:    "abc123  sdd-memory_1.0.0_linux_amd64.tar.gz\n",
			filename:   "sdd-memory_1.0.0_linux_amd64.tar.gz",
			wantDigest: "abc123",
		},
		{
			name: "finds correct entry among multiple",
			content: "aaa111  sdd-memory_1.0.0_linux_amd64.tar.gz\n" +
				"bbb222  sdd-memory_1.0.0_linux_arm64.tar.gz\n" +
				"ccc333  sdd-memory_1.0.0_windows_amd64.zip\n",
			filename:   "sdd-memory_1.0.0_linux_arm64.tar.gz",
			wantDigest: "bbb222",
		},
		{
			name:          "missing filename returns error",
			content:       "abc123  sdd-memory_1.0.0_linux_amd64.tar.gz\n",
			filename:      "sdd-memory_1.0.0_darwin_arm64.tar.gz",
			wantErrSubstr: "not listed in checksums.txt",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := sddMemoryExpectedChecksumFor(tt.content, tt.filename)
