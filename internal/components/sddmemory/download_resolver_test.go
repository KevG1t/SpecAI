...
// TestResolveSDDMemoryInstallNonBrewReturnsError verifies that after the fix,
// resolver.go only handles brew. Non-brew cases handled by DownloadLatestBinary.
// This is tested indirectly via the sddmemory package — after our change,
...
func TestInstallCommandBrewStillWorks(t *testing.T) {
	...
	for _, cmd := range cmds {
		for _, arg := range cmd {
			if strings.Contains(arg, "sdd-memory") {
				found = true
			}
		}
	}
	if !found {
		t.Fatalf("InstallCommand(brew) commands don't reference sdd-memory: %v", cmds)
	}
}
