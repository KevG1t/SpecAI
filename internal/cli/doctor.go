...
// checkStateJSON validates ~/.specai/state.json and agent config dirs.
func checkStateJSON(homeDir string) CheckResult {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return CheckResult{
				...
				Remedy: "Run 'specai install' to create initial state",
			}
		}
		return CheckResult{
			Name:   name,
			...
			Remedy: "Delete or repair " + statePath + ", then re-run 'specai install'",
		}
	}
	...
	if len(s.InstalledAgents) == 0 {
		return CheckResult{
			...
			Remedy: "Run 'specai install' to configure agents",
		}
	}
	...
	if len(missing) > 0 {
		return CheckResult{
			...
			Remedy: "Run 'specai sync' to restore missing config files",
		}
	}
...
// checkDiskSpace reports free space on the ~/.specai filesystem.
func checkDiskSpace(homeDir string) CheckResult {
	const name = "disk:space"
	dir := filepath.Join(homeDir, ".specai")
