...
func TestComponentApplyStepOpenClawWorkspaceScopedInjections(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			fileName:  "AGENTS.md",
			marker:    "<!-- specai:sdd-orchestrator -->",
		},
	}
...
func assertOpenClawInstructionsInWorkspace(t *testing.T, workspace string) {
	...
	if !strings.Contains(soulText, "specai:persona") || !strings.Contains(soulText, "Senior Architect") {
		t.Fatalf("active workspace SOUL.md missing SpecAI persona; got:\n%s", soulText)
	}
}
