...
func RunInstall(args []string, detection system.DetectionResult) (InstallResult, error) {
	...
	// When the user ran `specai install --agent X` (explicit agent flag),
...
type prepareBackupStep struct {
	...
	// appVersion is the specai version that created this backup.
			...
			Run: func(context.Context) error {
				return fmt.Errorf(
					"Antigravity and Gemini CLI write rules to ~/.gemini/GEMINI.md\n" +
						"Antigravity intentionally uses the Gemini-compatible global prompt surface; the last synced SDD orchestrator owns the shared specai:sdd-orchestrator section.\n" +
