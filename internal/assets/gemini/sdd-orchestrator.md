...
- Always add to sub-agent prompt: `"If you make important discoveries, decisions, or fix bugs, save them to sdd-memory via mem_save with project: '{project}'."`
...
When launching sub-agents for SDD phases with sdd-memory mode, pass these exact topic_keys as artifact references:
...
Convention files under `~/.gemini/skills/_shared/` (global) or `.agent/skills/_shared/` (workspace): `sdd-memory-convention.md`, `persistence-contract.md`, `openspec-convention.md`.
