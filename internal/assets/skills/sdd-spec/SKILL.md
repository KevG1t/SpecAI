...
- **hybrid**: Follow BOTH conventions — persist to SddMemory (single concatenated artifact) AND write domain files to filesystem.
...
**IF mode is `sdd-memory`:** Existing specs were already retrieved from SddMemory in the Persistence Contract. Skip filesystem reads.
