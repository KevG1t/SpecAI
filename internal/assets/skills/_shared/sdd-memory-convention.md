# sdd-memory Artifact Convention (reference documentation)

NOTE: Critical sdd-memory calls (`mem_search`, `mem_save`, `mem_get_observation`) are inlined directly in each skill's SKILL.md. This document is supplementary reference — sub-agents do NOT need to read it to function.

## Naming Rules

ALL SDD artifacts persisted to sdd-memory MUST follow this deterministic naming:
...
Set `capture_prompt: false` when the sdd-memory tool schema supports it; if an older schema rejects or does not expose the field, omit it rather than failing.
...
sdd-memory auto-detects the project name from the git remote at MCP startup. The `--project` flag and `SDD-MEMORY_PROJECT` env var can override detection. All project names are normalized to lowercase and trimmed.
...
Same `topic_key` + `project` + `scope` → UPDATE (overwrite), not INSERT. Previous content is lost — `revision_count` increments but old content is NOT saved. This is by design — sdd-memory is working memory, not an audit trail. For iteration history or team collaboration, use `openspec` or `hybrid` mode.
