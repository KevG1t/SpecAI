...
- **hybrid**: Follow BOTH conventions — persist to SddMemory AND write `tasks.md` to filesystem. Retrieve dependencies from SddMemory (primary) with filesystem fallback.
...
**Change**: {change-name}
**Location**: `openspec/changes/{change-name}/tasks.md` (openspec/hybrid) | SddMemory `sdd/{change-name}/tasks` (sdd-memory) | inline (none)
