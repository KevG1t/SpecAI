...
const (
	envNoSelfUpdate   = "SPECAI_NO_SELF_UPDATE"
	envSelfUpdateDone = "SPECAI_SELF_UPDATE_DONE"
	envConfirmUpdate  = "SPECAI_CONFIRM_UPDATE"
)
...
// selfUpdate checks for and applies a specai update before normal dispatch.
// Returns nil on success or skip; errors are non-fatal (caller logs and continues).
//
// Guard evaluation order (per spec):
//  1. SPECAI_SELF_UPDATE_DONE=1 → skip (loop guard)
//  2. SPECAI_NO_SELF_UPDATE=1 → skip (opt-out)
...
func selfUpdate(ctx context.Context, version string, profile system.PlatformProfile, stdout io.Writer) error {
	...
	// Check for updates (only specai).
	results := updateCheckFiltered(ctx, version, profile, []string{"specai"})

	// Find the specai result.
	var target *update.UpdateResult
	for i := range results {
		if results[i].Tool.Name == "specai" {
			target = &results[i]
			break
		}
	}
	...
	// Guard: if SPECAI_CONFIRM_UPDATE=1, prompt the user before applying.
	...
	for _, r := range report.Results {
		if r.ToolName == "specai" && r.Status == upgrade.UpgradeSucceeded {
			succeeded = true
			break
		}
	}
