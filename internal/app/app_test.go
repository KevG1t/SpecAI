...
func TestListBackupsNewestFirst(t *testing.T) {
	home := t.TempDir()
	backupRoot := filepath.Join(home, ".specai", "backups")
...
func TestPersistAssignmentsNoOpWhenEmpty(t *testing.T) {
	...
	statePath := filepath.Join(home, ".specai", "state.json")
...
func TestVersionBeforeSystemGuards(t *testing.T) {
	...
	if !strings.Contains(buf.String(), "specai") {
		t.Error("version output should contain 'specai'")
	}
}
...
// an error whose message suggests running 'specai help'.
func TestUnknownCommandSuggestsHelp(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "specai help") {
		t.Error("unknown command error should suggest 'specai help'")
	}
}
...
func TestRunArgs_UpdateSkipsSelfUpdate(t *testing.T) {
	...
	updateCheckAll = func(context.Context, string, system.PlatformProfile) []update.UpdateResult {
		return []update.UpdateResult{
			{
				Tool:             update.ToolInfo{Name: "specai"},
